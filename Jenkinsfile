...
def cosignAttestAndVerifyAttestionBlob(helmChart, helmPredicateContents){
    ...
    withCredentials([file(credentialsId: 'cosign-key', variable: 'cosign_pvt')]) {
        sh("COSIGN_EXPERIMENTAL=1 COSIGN_PASSWORD='' cosign attest-blob --force --key '${cosign_pvt}'--predicate cosign-metadatafiles/helmChartPredicate-MetaData.json --type \"spdxjson\" ${helmChart} --output-signature ${helmChart}-predicate.sig --rekor-url 'https://rekor.sigstore.dev'")
    }
    withCredentials([file(credentialsId: 'cosign-pub', variable: 'cosign_pub_key')]) {
